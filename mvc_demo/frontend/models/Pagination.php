        ...
        if ($this->params['full_mode'] == FALSE) {
            for ($page = 1; $page <= $total_page; $page++) {
                ...
                if ($page == $current_page&&$page==1) {
                    $data .= "<a class='active' href=''>$page</a>";
                }
                ...
                if ($page == 1&&$page!=$current_page || $page == $total_page&&$page!=$current_page || $page  == $current_page - 1 || $page == $current_page + 1) {
                    $page_url = "index.php?controller=$controller&action=$action&page=$page";
                    $data .= "<a href='$page_url'>$page</a>";
                }
                //nếu là trang hiện tại thì sẽ ko có link
                else if ($page == $current_page&&$page!=1&&$page!=$total_page) {
                    $data .= "<a class='active' href=''>$page</a>";
                }
