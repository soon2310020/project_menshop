...
class ProductController extends Controller
{
      ...
      if (isset($_POST['submit2'])) {
          ...
          if (empty($this->error)) {
              ...
              if ($_FILES['avatar']['error'] == 0) {
                  $dir_uploads = __DIR__ . '/../assets/uploads/products_avatar';
              ...
              header('Location: index.php?controller=product&action=createSizeColor');
              exit();
          }
      }
  ...
  public function createSizeColor()
  {
      $product_model= new Product();
      $product = $product_model->getLatest();
//      echo "<pre>";
//      print_r($product);
//      echo "</pre>";
      if (isset($_POST['submit']))
      {
          ...
          {
              $this->error="Hãy nhập size hoặc color";
          }
          else if (empty($amount))
          {
              $this->error="hãy nhập số lượng ";
          }
          else
          {
              $regex="/^#[0-9a-fA-F]{6}$/";
              $is_color=preg_match($regex,$color);
              if ($is_color==0)
              {
                  $this->error=" Hãy nhập vào 1 mã màu";
              }
          }
          if (empty($this->error))
          {
              ...
              $product_model->size=strtoupper($size);
              $product_model->id=$product['id'];
              $is_insert=$product_model->insertSizeColor();
//              var_dump($is_insert);
              if ($is_insert==false)
              {
                  $this->error="Đã tồn tại  màu sắc và size tương tự";
              }
              else
              {
                  $_SESSION['success']="Thêm size và màu cho sản phẩm thành công";
                  header("location:index.php?controller=product&action=createSizeColor");
                  exit();
              }
      ...
      $this->content = $this->render('views/products/createSizeColor.php');
      require_once 'views/layouts/main.php';
  ...
  public function cancel()
  {
      ...
      $_SESSION['success']="Đã hủy tạo sản phẩm";
  ...
  {
      if (!isset($_GET['id']) || !is_numeric($_GET['id'])||!isset($_GET['color'])||!isset($_GET['size'])) {
          $_SESSION['error'] = 'Sản phẩm xóa không hợp lệ';
          header('Location: index.php?controller=product');
          exit();
      }
      $id = $_GET['id'];
      $color = str_replace('z','#',$_GET['color']);
      $size = $_GET['size'];
      $product_model = new Product();
      $product = $product_model->getColorSize2($id,$color,$size);
      $product2 = $product_model->getById($id);
...
//      echo "</pre>";
    ...
    if (isset($_POST['submit'])) {
      ...
      //xử lý validate
      if (empty($title)) {
        $this->error = 'Không để trống tên sản phẩm';
      }
      else if ($_FILES['avatar']['error'] == 0) {
        //validate khi có file upload lên thì bắt buộc phải là ảnh và dung lượng không quá 2 Mb
        $extension = pathinfo($_FILES['avatar']['name'], PATHINFO_EXTENSION);
      ...
      if (empty($this->error)) {
        ...
        $is_update = $product_model->update($id,$color,$size);
        if ($is_update) {
  ...
  public function delete()
  {
    ...
    $size = $_GET['size'];
    $product_model = new Product();
    $is_delete = $product_model->delete($id,$color,$size);
    if ($is_delete||$product_model->getColorSize($id)==0)
    {
        ...
        $dir_uploads = __DIR__ . '/../assets/uploads/products_avatar/';
        @unlink($dir_uploads . '/' . $filename);
        $is_delete=$product_model->delete2($id);
    }
    if ($is_delete) {
