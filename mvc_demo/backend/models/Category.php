...
//    echo "</pre>";
  ...
  public function getAllPagination($params = [])
  {
    ...
    $start = ($page - 1) * $limit;
      $query='';
      if (isset($params['query_additional']))
      {
          $query=$params['query_additional'];
      }
      $obj_select = $this->connection
          ->prepare("SELECT * FROM categories  where true $query LIMIT $start, $limit" );
...
//        $obj_select->bindParam(':start', $start, PDO::PARAM_INT);
  ...
  public function getTotalSearch($params=[])
  {
      $query='';
      if (isset($params['query_additional']))
      {
      ...
      $obj_select = $this->connection
          ->prepare("SELECT count * FROM categories  where true $query " );
